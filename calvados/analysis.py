...
def calc_wcn(comp,pos,fdomains=None,ssonly=True,r0=0.7):
    ...
    if ssonly:
        ssdomains = build.get_ssdomains(comp.name,fdomains)
        wcn = np.zeros((N))
        for i in range(N-1):
            for j in range(i+1,N):
                ss = False
...
def calc_rmsd(u,uref,select='all',f_out=None,step=1):
    ...
    u_mean = mda.Merge(u.atoms) # new universe from mean structure
    ...
    return Rref.rmsd.T,Rmean.rmsd.T,RMSFmean.rmsf

def calc_rg(u,ag,avg_only=True):
    """ Average and sem radius of gyration of atom group in trajectory """
    rg2s = []
    for t, ts in enumerate(u.trajectory):
        r = ag.radius_of_gyration() / 10. # in nm
        rg2s.append(r**2)
    rg2s = np.array(rg2s)
    rgs = np.sqrt(rg2s)
    ...
    rg = np.sqrt(rg2)
    rg_sem = 0.5 * rg2**(-0.5) * rg2_sem
    if avg_only:
        return rg, rg_sem
    else:
        return rgs, rg, rg_sem
...
def fit_scaling_exp(u,ag,r0=None,traj=True):
    ...
    # print(dij.shape)
    if r0 == None:
        (r0, v), pcov = curve_fit(scaling_exp,ij,dij)
    else:
        v, pcov = curve_fit(lambda x, v: scaling_exp(x,r0,v), ij, dij)
        v = v[0]
